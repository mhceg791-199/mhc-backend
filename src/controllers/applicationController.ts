...
export const submitApplication = async (req: Request, res: Response) => {
  try {
    ...
    // ⚡ رفع الملف أولاً (لازم ينتهي قبل الحفظ)
    const uploadResult: any = await uploadToCloudinary(req.file);
    const resumeUrl = buildDownloadUrl(uploadResult, ENV.CLOUDINARY_CLOUD_NAME);

    console.log("📄 Final Resume URL:", resumeUrl);

    // ✅ الحفظ والإرسال بالتوازي (Promise.all)
    const [newApplication] = await Promise.all([
      ...
      sendApplicationEmail({
        ...
        // لو فشل الإيميل، نسجل الخطأ بس مانوقفش الـRequest
        console.error("❌ Email failed (non-blocking):", err);
      }),
    ]);

    // ⚡ رد فوري على المستخدم
    res.status(201).json({
      message: "Application submitted successfully",
      data: newApplication,
    });
